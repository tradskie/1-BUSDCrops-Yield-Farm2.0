...
const farmerAddress = "0xE1Ea8A0158E7a3FF00FFc586a6a733844753A19b" // mainnet contract
const tokenAddr = "0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47" // avax
...
function refreshData() {
  ...
  tokenContract.methods
    .allowance(currentAddr, farmerAddress)
    .call()
    .then((result) => {
  ...
  farmerContract.methods
    .getUserActiveInvestments(currentAddr, 0)
    .call()
    .then((result) => {
      ...
      if (amt > 0) {
        ...
        farmerContract.methods
          .getUserDividends(currentAddr, 0)
          .call()
          .then((dividends) => {
  ...
  farmerContract.methods
    .getUserActiveInvestments(currentAddr, 1)
    .call()
    .then((result) => {
      var amt = web3.utils.fromWei(result)
      if (amt > 0) {
        ...
        farmerContract.methods
          .getUserDividends(currentAddr, 1)
          .call()
          .then((dividends) => {
            var divs = web3.utils.fromWei(dividends)
  ...
  farmerContract.methods
    .getUserActiveInvestments(currentAddr, 2)
    .call()
    .then((result) => {
      var amt = web3.utils.fromWei(result)
      if (amt > 0) {
        ...
        farmerContract.methods
          .getUserDividends(currentAddr, 2)
          .call()
          .then((dividends) => {
            var divs = web3.utils.fromWei(dividends)
  ...
  farmerContract.methods
    .getUserActiveInvestments(currentAddr, 3)
    .call()
    .then((result) => {
      var amt = web3.utils.fromWei(result)
      if (amt > 0) {
        ...
        farmerContract.methods
          .getUserDividends(currentAddr, 3)
          .call()
          .then((dividends) => {
            var divs = web3.utils.fromWei(dividends)
  ...
  farmerContract.methods
    ...
    .then((result) => {
      ...
      var x = setInterval(function () {
        ...
        document.getElementById("claim-timer").innerHTML =
    ...
    userAmountOfDeposits((result) => {
      if (result && result > 0) {
        ...
        for (let i = 0; i < result; i++) {
          promises.push(
            farmerContract.methods.getUserDepositInfo(currentAddr, i).call()
          )
        }
  ...
  getUserDeposits(function (results) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const dateEnd =
        deposit.finish > 2000000000
          ? "Never"
          : new Date(deposit.finish * 1000).toLocaleString()
      const markup = `<tr><td>${i + 1}</td><td>Plan ${
        +deposit.plan + 1
      }</td><td>${deposit.percent / 10}%</td><td>${Number(
        web3.utils.fromWei(deposit.amount)
      ).toFixed(2)} AVAX</td><td>${dateStart}</td><td>${dateEnd}</td></tr>`
  ...
  farmerContract.methods
    ...
    .then((result) => {
      ...
      if (round) {
        ...
        farmerContract.methods
          .ticketOwners(round, currentAddr)
          .call()
          .then((numTix) => {
            if (numTix) {
        ...
        if (round >= 1) {
          farmerContract.methods
            .getLotteryHistory(round - 1)
            .call()
            .then((winner) => {
              var winnerAddress = winner.winnerAddress
              let shortenedAddr = winnerAddress.replace(
                winnerAddress.substring(6, 38),
                "***"
              )
